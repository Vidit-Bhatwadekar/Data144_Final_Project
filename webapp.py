...
Using a dataset of 6000 movie descriptions, we built a Movie2Vec model which embeds each movie as a 100 domensional vector.
...
df = pd.read_csv("./data/mpst_full_data_with_vec.csv", lineterminator="\n", index_col=0)
...
def find_similar_movies(description):
    ...
    closest_titles = [(df.loc[n, "original_title"], df.loc[n, "genre"]) for n in [index[0] for index in closest]]
...
def find_similar_movies_addition(positive_list, negative_list):
    ...
    for movie in positive_list:
        list_of_words = df[df["original_title"]==movie]["list_overview"]
        vec = model.infer_vector(list_of_words)
        add_vecs.append(vec)
...
with st.container():
    ...
    add_list = st.multiselect("Select the movies you'd like to add", df["original_title"])
    add_list = list(add_list)
    sub_list = st.multiselect("Select the movies you'd like to subtract", df["original_title"])
